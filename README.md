...
- **架构统一**：彻底移除 TKinter 旧架构，项目从双轨制统一为纯 PyQt6，删除约 8.9K 行死代码。
- **补丁安全加固**：新增 AST 验证器 + 三级权限沙箱，任意脚本不再可当作补丁执行。
- **检测器引擎跃升**：新增 5 种检测器（启动崩溃/世界加载/实体更新/OptiFine/LRU 缓存），诊断引擎改为并行执行，F1 从 71.8% 提升至 98.4%。
- **仪表盘实时监控**：异常检测 + 告警冷却（1296 条/分降至 ~18 条/分）+ UUID 消除 ID 冲突。
- **启动加速**：BrainCore 延迟初始化，点击启用时才加载，冷启动减少 2-30s。
- **UI 扁平化重设计**：600+ 行 CSS 重构为简洁扁平配色，5 色 AI 状态标签。
- **安全加固**：`/ready` 信息泄露修复，`/metrics` 访问控制，硬编码路径修复。

完整记录请查看 [CHANGELOG.md](CHANGELOG.md)。
已知问题与修复请查看 [KNOWN_ISSUES.md](KNOWN_ISSUES.md)。
